...
accperp.u <- logistic.acc.perp(npf, 100, y.binary, cv1$lambda.min)
acc.u <- accperp.u[[1]]
perp.u <- accperp.u[[2]]
```

```{r}
acc_all <- c(acc.s, acc.u)
breaks_acc <- seq(
  from = min(acc_all),
  to   = max(acc_all),
  length.out = 20
...
hist1.s <- hist(acc.s, breaks = breaks_acc, plot=FALSE)
hist2.s <- hist(perp.s, plot=FALSE)

hist1.u <- hist(acc.u, breaks = breaks_acc, plot=FALSE)
hist2.u <- hist(perp.u, plot=FALSE)

ylim_acc <- range(hist1.s$counts, hist1.u$counts)
ylim_perp <- range(hist2.s$counts, hist2.u$counts)

par(mfrow=c(1, 2))

plot(hist1.s, col = rgb(1,0,0,0.4), xlab="Accuracy", main="Accuracy", ylim = ylim_acc)
plot(hist1.u, col = rgb(0,0,1,0.4), xaxt='n', yaxt = 'n', add=TRUE)
...
plot(hist2.s, col = rgb(1,0,0,0.4), xlab="Perplexity", main="Perplexity", ylim = ylim_perp)
...
for (i in 1:J) {
  ...
  acc.multi1.u[i] <- mean(valid.pred == y.valid)
}
```

```{r}
acc_all2 <- c(acc.multi1, acc.multi1.u)
breaks_acc <- seq(
  from = min(acc_all2),
  to   = max(acc_all2),
  length.out = 20
...
hist2.s <- hist(acc.multi1, plot=FALSE, breaks=acc_all2)
hist2.u <- hist(acc.multi1.u, plot=FALSE, breaks=acc_all2)

ylim_acc2 <- range(hist2.s$counts, hist2.u$counts)

plot(hist2.s, col = rgb(1,0,0,0.4), xlab="Accuracy", main="Expected accuracy from multinomial classification", ylim = ylim_acc2, xlim = c(0.56, 0.74))
plot(hist2.u, xaxt='n', yaxt = 'n', col = rgb(0,0,1,0.4), add=TRUE)
...
for (i in 1:J) {
  ...
  acc.multi2.s[i] <- mean(y.valid == res$predicted)
}
```
...
for (i in 1:J) {
  ...
  acc.multi2.u[i] <- mean(y.valid == res$predicted)
}
```

```{r}
hist3.s <- hist(acc.multi2.s, plot=FALSE)
hist3.u <- hist(acc.multi2.u, plot=FALSE)

ylim_acc3 <- range(hist3.s$counts, hist3.u$counts)

plot(hist3.s, col = rgb(1,0,0,0.4), xlab="Accuracy", main="Expected accuracy from logistic one-vs-all", ylim = ylim_acc3, xlim = c(0.56, 0.74))
plot(hist3.u, xaxt='n', yaxt = 'n', col = rgb(0,0,1,0.4), add=TRUE)
...
```{r}
rows <- c("Scaled LR", "Unscaled LR", "Scaled LR (one-vs-all)", "Unscaled LR (one-vs-all)", "Scaled Multinomial", "Unscaled Multinomial")

accs.b <- c(mean(acc.s), mean(acc.u), NA, NA, NA, NA)
accs.m <- c(NA, NA, mean(acc.multi2.s), mean(acc.multi2.u), mean(acc.multi1), mean(acc.multi1.u))
perps <- c(mean(perp.s), mean(perp.u), NA, NA, NA, NA)
kaggle <- c(0.74878, 0.74705, NA, 0.74360, 0.74878, 0.74705)
