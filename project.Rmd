...
for (i in 1:J) {
  ...
  acc.binary.lr[i] <- mean(pred == valid$class4)
...
```
We can expect our accuracy to be between 0.86 - 0.9 on average, and perplexity around 4 - 8.
...
```{r}
n <- floor(0.5*nrow(train.multi))
J <- 30
acc.multi1 <- numeric(J)

for (i in 1:J) {
  picked <- sample(seq_len(nrow(train.multi)), size = n)

  train <- train.multi[picked, ]
  valid <- train.multi[-picked, ]
  ...
  acc.multi1[i] <- mean(valid.pred == valid$class4)
}
```
...
### Logistic regression (one-vs-one, one-vs-all)
...
test.x <- model.matrix(~ ., data=test)[, -1]

classes <- predict(model.multi, newx = test.x, type = "class")
ps <- predict(model.binary, newx = test.x, type = "response")

result_df <- data.frame(id=test.ids, class4=classes, p=ps)
...
```{r}
rows <- c("Logistic Regression", "Multinomial Regression")

accs.b <- c(mean(acc.binary.lr), NaN)
accs.m <- c(NaN, mean(acc.multi1))
perps <- c(mean(perp.lr), NaN)
kaggle <- c(0.75559, 0.75559)
